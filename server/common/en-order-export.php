...
namespace EnFreightviewOrderExport;

/**
 * Get and save settings.
 * Class EnFreightviewOrderExport
 * @package EnFreightviewOrderExport
 */
if (!class_exists('EnFreightviewOrderExport')) {
    ...
    class EnFreightviewOrderExport
    {
        ...
        public function en_orders_sending($orders, $en_orders_ids)
        {
            $post_data = [
                ...
                'licenseKey' => get_option('en_connection_settings_license_key_freightview'),
                'currencyUnit' => get_option('woocommerce_currency'),
                'platform' => 'wordpress',
                'orders' => $orders,
            ];

            $response = \EnFreightviewCurl\EnFreightviewCurl::en_freightview_sent_http_request(EN_FREIGHTVIEW_ORDER_EXPORT_HITTING_URL, $post_data, 'POST', 'Export Order');
